 ...
 * POST /api/cron/sync
 *
 * Regelmässiger Sweep über ALLE verbundenen Quellen — Polar und Google.
 * Hiess früher /api/cron/polar-sync; dieser Pfad existiert noch als
 * Weiterleitung, bis der Coolify Scheduled Task umgehängt ist.
 *
      ...
      try {
        ...
        if (isSlotDue(user.dailySyncedAt, DAILY_SYNC_HOURS, now)) {
          try {
            ...
            console.log(
              `[cron/sync] daily slot done user=${user.id} days=${days}`
            );
          } catch (e) {
            console.error(`[cron/sync] daily failed user=${user.id}:`, e);
          }
          try {
            await syncPhysicalInfo(user.id, user.polarToken);
          } catch (e) {
            console.error(`[cron/sync] physical-info failed user=${user.id}:`, e);
          }
        }
        ...
        if (isSlotDue(user.sleepSyncedAt, SLEEP_SYNC_HOURS, now)) {
          try {
            ...
            await db
              ...
              .where(eq(users.id, user.id));
            console.log(
              `[cron/sync] sleep slot done user=${user.id} sleep=${r.sleepSynced} nights=${r.nightsSynced}`
            );
          } catch (e) {
            console.error(`[cron/sync] sleep failed user=${user.id}:`, e);
          }
        }
      } catch (e) {
        if (e instanceof PolarAuthError) {
          ...
          reauthNeeded++;
          console.warn(`[cron/sync] token rejected user=${user.id} — reconnect needed`);
          continue;
        }
        const msg = e instanceof Error ? e.message : String(e);
        errors.push(`user=${user.id}: ${msg}`);
        console.error(`[cron/sync] sync failed user=${user.id}:`, e);
      }
    }

    console.log(
      `[cron/sync] sweep done: candidates=${toSync.length} usersSynced=${usersSynced} activitiesSynced=${activitiesSynced} reauthNeeded=${reauthNeeded} errors=${errors.length}`
    );
  } catch (err) {
