...
export default async function ActivityClassicPage({
  params,
}: {
  ...
  const routeData = (activity.routeData as { lat: number; lng: number; elevation?: number | null }[]) || [];
  const heartRateData = (activity.heartRateData as { time: string; bpm: number }[]) || [];
  const speedData = (activity.speedData as { time: string; speed: number }[]) || [];
